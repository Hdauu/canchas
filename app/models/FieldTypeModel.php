...
class FieldType
{
    ...
    public function deleteFieldType($conn, $id) {
        // Eliminar los registros de la tabla 'field' que referencian a este tipo de cancha
        $stmt = $conn->prepare("DELETE FROM field WHERE FieldType_id = :id");
        $stmt->bindParam(':id', $id);
        $stmt->execute();
    
        // Ahora eliminar el tipo de cancha
